...
% Help LaTeX break long \texttt{} tokens (filenames, paths, model names)
% so they do not bleed across the column rule in two-column layout.
...
% that folder to Overleaf alongside this file to render the qualitative tables.
...
\affiliation{%
  ...
  \postcode{78412}
  \country{USA}}
\author{Minhua Huang, Ph.D.}
\authornote{Associate Professional Professor; course instructor and project advisor.}
...
\affiliation{%
  ...
  \postcode{78412}
  \country{USA}}
\renewcommand{\shortauthors}{Pagadala, Umapathy, and Dr.\ Huang}
...
\begin{document}
\begin{abstract}
A conversational system that listens to only one channel of human affect tends to misread the room. Words can hide what someone is feeling, and a face can flatly contradict the words. We build a bimodal pipeline that takes both at once. A RoBERTa text classifier (fine-tuned on GoEmotions) and a ResNet-50 face classifier (FER2013-aligned) each emit a seven-way affect distribution, and a single trust weight $\alpha\in[0,1]$ blends them via
$b^*=\arg\max_k\bigl(\alpha P_\text{text}(k)+(1\!-\!\alpha)P_\text{face}(k)\bigr)$.
The fused class plus the user's utterance is passed to FLAN--T5, which writes a short empathetic reply. A Streamlit interface displays the two probability vectors, the fused decision, an explicit affective-dissonance flag, and per-stage timings, so a reviewer can watch the system reason rather than just trust the output. We report unimodal and fused accuracy on the full GoEmotions and FER2013 test splits, a FER2013 fine-tune that lifts visual top-1 from 23.1\% to 68.5\%, real-image and synthetic dissonance evaluations, and a latency breakdown. Affect inference takes about 38\,ms on Apple MPS, comfortably under the 400\,ms Doherty threshold, while end-to-end response generation with FLAN--T5 (base) lands at 942\,ms.
\end{abstract}
...
A dialogue system that only sees text is easy to fool. Sarcasm reads as cheerful, a polite ``I'm fine'' hides distress, and a deadpan one-liner gives almost no signal at all. Vision alone has the mirror weakness: the same neutral face shows up in good lighting, bad lighting, off-axis poses, and partial occlusion. Either way, the system has to guess, and a wrong guess in the empathy loop costs user trust quickly~\cite{card1983human}. Reading both channels together gives the model a second opinion when both are available, and a way to flag the cases where the two opinions clash.

\subsection{Related work}
Facial expression recognition and text-based emotion classification are each mature on their own. Tight \emph{coupling} of the two in a shared seven-class label space with an exposed trust weight is comparatively rare at course scale. We borrow the GoEmotions corpus of Demszky et~al.~\cite{demeszky2020goemotions} for text, the FER2013-style ResNet recipe of Goodfellow et~al.~\cite{goodfellow2013challenges} for vision, and the instruction-tuned T5 family of Chung et~al.~\cite{chung2022scaling} for the empathetic response. Latency expectations are framed against the 400\,ms Doherty interaction threshold from Card, Newell, and Moran~\cite{card1983human}.

\subsection{Approach and contributions}
The pipeline mirrors the course architecture (Fig.~\ref{fig:pipeline}). A transformer text sensor (RoBERTa fine-tuned on GoEmotions~\cite{demeszky2020goemotions}) emits $P_\text{text}$ over the seven FER classes; a ResNet-50 face sensor~\cite{goodfellow2013challenges} emits $P_\text{face}$ over the same seven classes; a late fusion (Eq.~\ref{eq:main}) blends them with a trust weight~$\alpha$; and a FLAN--T5 generator~\cite{chung2022scaling} turns the fused class into a short empathetic reply. Five concrete contributions follow. We define a deterministic surjection from the 28 GoEmotions labels into the 7 FER classes. We ship an end-to-end Streamlit demo that exposes every intermediate signal so a reviewer can probe the system rather than just consume its output. We provide a separate FER2013 fine-tuning workflow that takes the face branch from a near-chance 23.1\% to 68.5\% top-1 without altering the deployed code path. We package scripted ablation, $\alpha$-sensitivity, dissonance, and latency benchmarks tied to a shared evaluation harness. And we discuss, plainly, the places each component still breaks.
...
\begin{equation}
  ...
  \label{eq:main}
\end{equation}
Setting $\alpha\!=\!1$ recovers the text-only ablation, $\alpha\!=\!0$ recovers the face-only ablation, and $\alpha\!=\!0.5$ is the balanced setting we treat as the system default. Sweeping $\alpha$ across $[0.2,0.8]$ probes how brittle the fused decision is to the trust setting. A sample is flagged as showing \emph{affective dissonance} whenever $\arg\max P_\text{text}\neq\arg\max P_\text{face}$; this flag is what the Streamlit interface surfaces to the user as a coloured badge.

\section{Method}
\subsection{Mapping 28 GoEmotions labels to FER\texorpdfstring{$7$}{\,7}}
GoEmotions ships with 28 fine-grained emotion strings. The visual branch only resolves seven. We collapse 28 down to 7 with a hand-coded surjection that lives in the codebase as a single Python dictionary: every GoEmotions string maps to exactly one of \{\,Angry, Disgust, Fear, Happy, Sad, Surprise, Neutral\,\}. GoEmotions is multilabel and uses sigmoid scores, so we \emph{sum} the sigmoid mass landing in each FER-7 bin and renormalize to a proper distribution. That keeps the relative confidence of fine-grained labels (for instance, \emph{annoyance} adding to \emph{anger}) when several map to the same bin.
...
The default visual checkpoint is fine for a live demo, but it is clearly the weakest part of the pipeline on FER2013 test data: an AffectNet-trained model asked to score 48$\!\times\!48$ grayscale frames is operating well outside its training distribution. We fine-tuned the same ResNet-50 head on the \path{AutumnQiu/fer2013} train split with a held-out validation slice and standard cross-entropy. Table~\ref{tab:finetune} shows the result. Full-test top-1 accuracy moves from 23.1\% to 68.5\%, almost a three-fold improvement and roughly the difference between near-chance and a usable face signal. We deliberately keep the fine-tuned weights as a separate checkpoint from the deployed demo, so every baseline number in this report stays reproducible without extra setup; \path{scripts/eval_fer_checkpoint.py} loads the new file directly when the reader wants to verify the fine-tuned numbers.
...
Dissonance, here, is the case where the text and the face disagree about what the user feels. Two complementary probes test it. The real-image probe pairs every FER2013 test image with a deliberately mismatched template sentence and treats the FER2013 label as the intended affect; the fused decision should ideally recover that intended label, but the baseline face branch is only 23.1\% accurate on FER2013, so most of the time the system follows the misleading text rather than the noisy face. The synthetic probe replaces the face branch with a one-hot vector for the intended class, isolating the fusion arithmetic from any face-classifier noise: when one of the two inputs is perfect, $\alpha\!=\!0.5$ should, and does, recover the intended class even though the other input is wrong by construction. Table~\ref{tab:dissonance} lists both. The real-image row is the deployment-relevant result; the synthetic row is a sanity check on the fusion rule itself.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{3.4cm}cccc@{}}
    \toprule
    \textbf{Dissonant setting} & \textbf{$n$} & \textbf{$\alpha\!=\!1$} & \textbf{$\alpha\!=\!0.5$} & \textbf{$\alpha\!=\!0$} \\
    \midrule
    Real FER2013 + mismatched text   & 3589 & 4.0\%  & 4.3\%   & 23.1\% \\
    Synthetic one-hot + mismatched text & 100  & 3.0\%  & 100.0\% & 100.0\% \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:qualdissimages} drills into the real-image dissonant setting with the first 10 FER2013 test images, the same indices that contribute to the aggregate row above. The mismatched-text class~$j$ is fixed by the row index via $j = (y + 1 + (k\bmod 6)) \bmod 7$, so every row has a different gold/text pairing and we are not cherry-picking. Three patterns jump out. The text branch follows the planted misdirection on all 10 rows, which lines up with the $\sim$4\% aggregate $\alpha\!=\!1$ accuracy. The face branch (still the baseline AffectNet checkpoint here) collapses to \emph{Happy} on 7 of 10 rows, which is the visual-branch domain shift described in Section~\ref{sec:failmodes}. And the fused decision essentially tracks the text branch because the gold-versus-mismatch distinctions (Angry vs.\ Disgust, Sad vs.\ Surprise, and so on) are well below the resolution of a 23\%-accurate face signal. We revisit the same 10 rows with the fine-tuned face branch in Section~\ref{sec:finetuned}; the two tables share image indices so the reader can flip between them and compare the two visual backbones row by row.
...
Table~\ref{tab:qualimages} samples one real FER2013 test image per FER-7 gold class and pairs it with the matching congruent template text. We then read out the predicted FER-7 class at $\alpha\!=\!1$ (text only), $\alpha\!=\!0.5$ (fused), and $\alpha\!=\!0$ (face only) using the baseline ResNet-50. Two patterns stand out. The face branch is unreliable on FER2013, misclassifying 5 of the 7 images on its own, which is the per-row picture behind the 23.1\% aggregate in Table~\ref{tab:results}. The text branch, on the other hand, pulls the fused decision toward the correct gold class on 6 of 7 cases. The single failure is the Neutral row, where the template text itself contains positive-sounding language (``\texttt{feel pretty neutral}''), and the GoEmotions head reads it as Happy. That is precisely the kind of subtle text-side mistake the trust weight $\alpha$ is meant to surface rather than hide.
...
Once the fused class $b^{*}$ is decided, the user utterance $U$ and the FER-7 class string for $b^{*}$ are handed to FLAN--T5 (base, 220M) under a deterministic instruction prompt. Only $U$ and $b^{*}$ flow into the prompt; the probability vectors themselves are never exposed to the language model. That keeps the response stage in line with the assignment specification. The prompt is, lightly abbreviated:
...
\emph{Bimodal congruent (matched template text + real FER2013 image).} Re-running the unimodal/bimodal benchmark of Table~\ref{tab:results} with the fine-tuned face branch substituted in (\path{scripts/run_evaluation.py --finetune-ckpt}) gives Table~\ref{tab:resultsft}. The text branch is unchanged, as expected. The image branch tracks the FER2013 image-only fine-tune from Table~\ref{tab:finetune} (23.1\,\%~$\to$~68.5\,\%). And the fused $\alpha\!=\!0.5$ score climbs from 81.6\,\% to \textbf{87.8\,\%}, now clearly above the 82.6\,\% text branch on the same pairs. This is the first regime where adding the face actually \emph{helps} the fused decision rather than gently dragging it down.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{2.7cm}cccc@{}}
    \toprule
    \textbf{Visual branch} & \textbf{$n$} & \textbf{$\alpha\!=\!1$} & \textbf{$\alpha\!=\!0.5$} & \textbf{$\alpha\!=\!0$} \\
    \midrule
    Baseline (AffectNet)            & 3589 & 4.0\% & 4.3\%  & 23.1\% \\
    Fine-tuned (FER2013)            & 3589 & 4.0\% & \textbf{41.8\%} & \textbf{68.5\%} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\emph{Takeaway.} Fine-tuning the visual branch on FER2013 closes most of the AffectNet-to-FER2013 domain gap (23.1\,\%~$\to$~68.5\,\% face-only top-1 on $n\!=\!3589$) and turns the bimodal system from text-dominant into genuinely bimodal. On the matched congruent benchmark, $\alpha\!=\!0.5$ moves from 81.6\,\% (a hair below the 82.6\,\% text branch) to 87.8\,\% (clearly above text alone, Table~\ref{tab:resultsft}). On the engineered-dissonance benchmark, $\alpha\!=\!0.5$ now recovers about 42\,\% of cases where it previously recovered only $\sim$4\,\% (Table~\ref{tab:dissonanceft}). The cost is essentially running the fine-tune script we ship at \path{scripts/finetune_fer2013.py} and storing one extra checkpoint file; nothing in the deployed app code changes beyond which file the vision sensor loads at startup.

\subsection{Limitations}
This study is incomplete in three places, and we want to be plain about them. The matched-template bimodal benchmark is a probe, not a natural multimodal corpus; it measures whether aligned text and a face estimate can be fused consistently, not how well the system handles real spontaneous speech. The real-image dissonance probe is closer to deployment but still uses scripted text templates instead of utterances from actual participants. The visual branch is bounded by FER2013 label noise and by the AffectNet-to-FER2013 domain shift in Section~\ref{sec:failmodes}; the deployed FLAN--T5 (base) generator is intentionally minimal and would clearly benefit from a $\geq$1B-parameter instruction-tuned backbone, especially on the Sad and Surprise rows of Table~\ref{tab:qualresponses} where the small model patterns into multiple-choice or paraphrase formats. And this repository does not include a formal participant Likert study; the HCI component is best read as expert review plus a ready-to-run study template under \path{docs/HCI_STUDY_TEMPLATE.md}.

\subsection{Reproducibility checklist}
Every number in this report is produced by a script that lives in the repository. \path{scripts/run_evaluation.py} produces Table~\ref{tab:results} and the baseline row of Table~\ref{tab:finetune}. \path{scripts/finetune_fer2013.py} together with \path{scripts/eval_fer_checkpoint.py} produces the fine-tuned row of Table~\ref{tab:finetune}. \path{scripts/eval_dissonant_fer2013.py} and \path{scripts/build_dissonant_eval_csv.py} together with \path{scripts/run_ablation.py} produce Table~\ref{tab:dissonance}. \path{scripts/benchmark_latency.py} produces Table~\ref{tab:latency}. \path{scripts/build_qualitative_table.py} produces Tables~\ref{tab:qualimages} and~\ref{tab:qualresponses}, and \path{scripts/build_dissonant_qualitative_table.py} produces Tables~\ref{tab:qualdissimages} and~\ref{tab:qualdissimagesft}. Every script reads the same \texttt{.env}-supplied HuggingFace token and writes both a JSON summary and a LaTeX fragment to \path{docs/eval_output/}.
